...
- **커밋 단위는 "주제 하나"**: 코드와 **그 변경을 설명하는 문서(docs/NN)를 한
  커밋에** 담는다. 메시지 본문에 배경·실측 수치·교훈을 적어, `git log`만 읽어도
  무슨 일이 있었는지 알 수 있게 한다.
- **예외 — §11 갱신만 별도 docs 커밋으로**: 마일스톤 커밋을 먼저 하고, 그 해시를
  `docs: CLAUDE.md §11 갱신 — … (해시)` 커밋으로 기록한다. 마일스톤 커밋 안에 §11을
  넣으면 자기 해시를 미리 알 수 없어 기록이 어긋난다 (finish-milestone 스킬 참조).
  **이 예외는 §11에만 적용된다** — 설계 문서(docs/NN) 갱신까지 따로 떼지 않는다.
  (실사고 2026-09-02: 이 규칙을 모든 문서에 적용해 미push 커밋 25개가 코드·문서
  핑퐁으로 쌓였고, 같은 주제가 4개로 흩어져 이력을 읽을 수 없었다. 8개로 재정리.)
- **push 전에 이력을 한 번 본다**: `git log origin/main..HEAD --oneline`으로
  주제가 흩어졌는지 확인하고, 흩어졌으면 정리한 뒤 push한다. 정리는 **미push
  커밋만** 대상으로 하고(원격 이력 재작성 금지), 백업 브랜치를 만든 뒤
  **정리 전후 트리 해시**(`git rev-parse HEAD^{tree}`)가 같은지로 무손실을 확인한다.
...
- ✅ **검증 대격변 + 앰비소닉 규명 + 빔포밍** (2026-09-02~19, **상세: docs/21 §3-13**):
  - **"216/216 100%" 정정** — 자동 기준 통과율이었고 실제는 high 1,298/2,160
    (60.1%), 그 안에서 위치 정확도 94%(±0.05초). 발각 경로는 사용자 청취
    (030이 5초인데 1.83초 검출). "정확도"와 "사용 가능 비율"을 분리해 말할 것.
    **2,160개 전부 비프음 실재** — 미검출 677개는 negative 샘플로 쓰면 안 됨.
  - **마이크가 앰비소닉(H3-VR AmbiX)** — 채널 순서 **W·Y·Z·X**(헤더 실측),
    채널 상관 0·딜레이 무의미가 정상. 게인 10/100(공장 초기값, 파일에 미기록).
    **검출을 좌우하는 건 게인이 아니라 타겟 대역 SNR**(3일차 최저 레벨인데
    SNR 44dB로 99.4% vs 4일차 12.7dB로 30.4%). docs/18 §5~§6.
  - **빔포밍 적용** `36055fd` — 실데이터로 검증(미검출 40개 중 21개 복구,
    677개 중 ~355개 추정). `beam=None` 기본이라 기존 동작 100% 동일.
    **NAS 배포 미실시** — 절차는 docs/21 §3-13(f).
  - 다중 특징 판정은 구현했으나 **미채택** `a54ee0a`(청취 정답 18개 대비
    무의미한 개선). 결론엔 애매 구간 청취 정답 30~50개 필요.
  - 라벨 v1 `backend/labels/beep_labels_v1.json`(등급은 청취 미검증 초안).
    청취 시트 2,160행은 `~/Desktop/청취검증_시트.csv`(**저장소 밖**, 연구원 작성 중).
