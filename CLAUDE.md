...
- ✅ **NAS 실배포·검증 완료** 2026-08-28: 폴더 개명(`03_Experiment` →
  `03_DashBoard`)·계정 변경(`AURA` → `aura_admin`)에 맞춰 재배포.
  8개 dataset 무결성 통과, 실제 100초 원본에서 **진짜 비프음 10개를
  전부 정확히 검출**(오프셋 표준편차 0.09초, 기존 event_detection과
  같은 위상). **다만 오탐 10개 동반** — 실제 주차장 환경의 다른 소리로,
  사용자 결정에 따라 육안 확인 보조로 그대로 사용(개선은 파라미터 조정
  여지 있음). 부수 관찰: 실데이터 실제 위상은 5.0초가 아니라 **5.85초**
  (녹음 시작 지연) — 위상 GT는 파일별 오프셋 중앙값 기준으로 볼 것.
- **최신 마일스톤 커밋**: `f47a0f8`(경계 오탐 수정), `5d54e88`(비프 대역
  기능 신설). 인프라는 `d9805dd`(3분할 재이전), `a0c25b7`(삭제 버그 수정).
- 남은 V2 자리: GitHub(Dataset 버전), AI Assistant, Auth.
- **잔여**: 1~4일차 NAS 데이터 사람 검수(docs/19) 미착수. Railway 실서버(project
  41/42/45)에 남은 검증용 잔존 데이터 정리 여부 미확인. 비프음 검출기
  오탐(실환경 100초당 약 10개) 개선 여부는 사용자 판단 대기.
