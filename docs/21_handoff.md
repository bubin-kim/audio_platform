...
**NAS 실서버 검증 결과**(dataset 1의 `260818_001.WAV`, 100초):
- 진짜 비프음 **10개를 전부 정확히** 검출 — 5.87·15.85·25.92·35.85·
  45.85·55.81·65.62·75.76·85.76·95.84초. 이 10개의 오프셋 표준편차
  **0.09초**로, NAS 기존 `event_detection` 결과(5.851·15.836·25.913)와
  같은 위상이다.
- **다만 오탐 10개가 함께 잡힌다**(실제 주차장 환경의 다른 소리).
  화면에는 "검출 20개 · 불규칙"으로 표시된다. 합성 신호·6초 조각에서는
  없던 문제로, **사용자 결정: 지금은 이대로 두고 육안 확인 보조로 사용**
  (오탐 개선은 필요하면 `k`·`min_gap_sec`·대역폭 파라미터로 조정).
