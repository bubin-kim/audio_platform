...
> **마지막 갱신**: 2026-08-26 (1~4일차 재전처리 중 Drive 용량 초과로 3·4일차
> 업로드 중단, 내일 NAS 연결 예정 시점)
...
### 3-6. 1~4일차 본수집 재전처리 — 1·2·3일차 부분 완료, **Drive 용량 초과로 중단** (2026-08-26)

새 폴더(`~/Downloads/beep_sound_dataset/mic1`·`mic2`)로 1~4일차 전량 재작업.
이전(§3-5)의 waste·손상·결측 파일이 이번엔 전부 정상으로 복구돼 있어
raw 매핑(mapping.py DAY1~DAY4)의 제외 항목(1일차 42·2일차 12·39·3일차mic1 24)을
**원래 조건으로 되살려** 포함했다 — 사용자가 매 항목 직접 확정.

**dataset id가 바뀌었다**: 기존 dataset 22(1일차)·23(2일차)를 삭제(재업로드 전
사용자 요청)한 뒤 재생성해 **1일차=29, 2일차=30**이 됐다(3일차=24, 4일차=25는
그대로). **삭제된 dataset id는 재사용 불가**라는 걸 이번에 직접 겪었다 — 재시도
스크립트가 옛 id(22/23)를 그대로 써서 전부 404로 실패했다가, 새 id로 고쳐서 재개.

**최종 상태(2026-08-26)**:

| dataset | 이름 | 원본 | 세그먼트 | 상태 |
|---|---|---|---|---|
| 29 | 1일차 | 54/54 | 527 | ✅ 완료 |
| 30 | 2일차 | 54/54 | 537 | ✅ 완료 |
| 24 | 3일차 | 49/54 | 483 | 🔶 **부분** — mic1 30개 전부 + mic2 19개(001~019). mic2 020~024·재커팅 필요분 남음 |
| 25 | 4일차 | 0/54 | 0 | ❌ **미착수** — 전량 Drive 403으로 실패 |

**중단 원인: Google Drive 저장 용량 초과로 추정(확정은 아님)**. 3일차 mic2
020번 근처부터 모든 업로드가 `403 Forbidden`
(`POST https://www.googleapis.com/upload/drive/v3/files`)으로 실패하기
시작했고, 이후 4일차 전체·재시도까지 전부 같은 에러. docs/20에 이미
"Drive 잔여 14GB(부족)"이라 기록돼 있었고, 오늘 1·2일차 재업로드만으로
약 9.4GB를 추가로 썼다(원본 6.2GB+세그먼트 3.5GB 추정) — 한도를 넘겼을
가능성이 높다. **정확한 사유는 미확인**: `backend/app/storage/drive.py`의
`_request()`가 `res.raise_for_status()`만 호출해 Google이 보낸 에러 본문
(`storageQuotaExceeded` 등 구체적 사유)을 로그에 남기지 않는다 — 다음에
같은 문제가 나면 그 응답 본문을 로깅하도록 고치는 게 먼저다.

**사용자 결정**: Drive 원인 조사는 여기서 멈추고(§9 "지금은 안 건드림"),
**다음 주 NAS 연결 후 이어서 처리**. dataset 24의 부분 데이터(483개)는
지우지 않고 그대로 둔다 — 이미 성공한 유효 데이터이므로.

**다음 세션이 할 일**:
1. NAS(`docs/20` 참조, DSM 관리자 권한 필요 — 8/25 세션에서 일반 계정이라
   Container Manager 접근 불가였음, 관리자 확보 여부 확인)
2. NAS 컨테이너 기동 확인
3. 3일차 나머지(mic2 020~024, 5개)와 4일차 전량(mic1 30개+mic2 24개=54개)을
   NAS 경유로 업로드+커팅 — **STORAGE_MODE를 NAS에 맞게 바꿀지, 아니면
   Railway는 그대로 두고 NAS는 별도 자체 호스팅으로 갈지 방향부터 확정**
   (docs/20은 NAS 자체 호스팅 — Railway 대체 — 을 전제로 쓰여 있음. 지금
   상황은 "Railway+Vercel은 유지, 저장소만 NAS로"일 수도 있어 재확인 필요)
4. mapping.py의 DAY3_MIC1[24]·DAY4_MIC1(32~61 오프싯 규칙)은 이미 검증
   완료 — 재사용 가능

---

## 4. 실서버 현재 상태 (2026-08-26)

| id | 프로젝트 | 커팅 파라미터 | 내용 |
|---|---|---|---|
| 41 | 경보음 탐지(파일럿 검증) | height=5 / gap=4 | 파일럿 004·005 |
| 42 | **beep sound** | height=3.0 / gap=8.0 | 목표 2160 — 1일차(29)·2일차(30) 완료, 3일차(24) 부분, 4일차(25) 미착수(§3-6) |
| 45 | 탐지 시험 (본녹음 샘플) | height=3.0 / gap=8.0 | dataset 21 "본녹음 샘플 (034·015·024·045·049)" 하나만 존재 — 49개 세그먼트 |
