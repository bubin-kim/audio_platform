...
**다음에 할 일**: ~~실서버(Drive) 반영 방법 결정~~ → **불필요해짐**(아래 참조).
로컬 결과(39개 조각)는 **사람이 검수 화면으로 전수 확인**하기로 확정
(2026-08-21, 사용자 결정 — 정확도 우선, 049는 놓친 1개 포함 원본 파형까지 훑을 것).
검수는 `http://localhost:3100` 프로젝트 id=6 → dataset id=7에서 docs/19 절차대로.

> **2026-08-21 방침 전환**: "다음 주부터 NAS에 업로드 예정이라 Drive에 올릴
> 필요 없어졌다"(사용자). 즉 이 4개 파일뿐 아니라 **앞으로 원본 업로드
> 경로 자체가 Drive(Railway)에서 NAS로 바뀐다.** §5-14의 Railway 300초
> 타임아웃 문제는 NAS 전환으로 자연히 우회되지만(로컬 네트워크라 별도
> 에지 프록시 타임아웃이 없을 가능성), **NAS 컨테이너 기동이 아직
> 미검증**이므로 실제로 그런지는 NAS 설치 후 확인해야 한다. §3-3의
> NAS 이전 항목이 이제 최우선이다.
...
| 45 | 탐지 시험 (본녹음 샘플) | height=3.0 / gap=8.0 | dataset 21=034(10조각, **최대값 집계로 재커팅 완료** 2026-08-21) · 26=045 · 27·28=수집중 |
...
14. **Railway 에지 프록시가 요청당 300초 하드 타임아웃을 건다** — 앱/`MAX_UPLOAD_MB`와
    ...
    `POST /api/uploads`는 원본 저장에 더해 **Drive 동기 업로드**(docs/13)까지 하나의
    요청 안에서 처리하므로, 실측 26MB=193초 → 58MB는 선형 외삽 시 300초를 넘는다.
    **10분 이상 녹음(대략 50MB+)은 실서버로 직접 업로드하면 안 된다.** 우회책:
    ① 로컬(`STORAGE_MODE=local`)에서 먼저 처리 ② 원본을 잘라 나눠 올리기
    ③ 청크/재개형(resumable) 업로드로 API를 바꾸기(구조 변경, 승인 필요) 중 택일.
    2026-08-21 015/024/045/049 업로드 시도로 발견 — §3-4 참조.
