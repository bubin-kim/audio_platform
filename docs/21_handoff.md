...
| NAS 이전 | 문서만 완료(docs/20) | 시놀로지 DS925+ 설치 후. **맥에 Docker가 없어 컨테이너 기동 미검증** |
| Railway → NAS 데이터 이전 | 미착수 | NAS 기동 확인 후 |
| 015·024·045·049 업로드 | **로컬에만 완료**(§3-4), **실서버 반영 안 됨** | 원본 업로드가 Railway 300초 타임아웃에 걸림(§5 신규 함정) — 해결책 결정 필요 |
...
**다음에 할 일**: 실서버 반영 방법을 결정한다 — 원본을 잘라 나눠 올리기 /
`STORAGE_MODE=local`로 임시 전환 후 업로드 / 그 외. 로컬 결과(39개 조각)는
그대로 참고용으로 남아 있다.
