...
> **마지막 갱신**: 2026-08-21 (커밋 `7d8689d` 시점)
...
### 3-2. 바로 다음에 할 일 (사용자 확인 대기 중)

**실서버 배포 + 재검증.** 사용자에게 "진행할까요?"를 물어둔 상태다.

```bash
# 저장소 루트에서
railway up --detach
railway logs                     # 마이그레이션 적용 줄 확인
```
배포 후 **id=45 "탐지 시험 (본녹음 샘플)"** 프로젝트에서 034를 재커팅해
로컬과 같은 **10조각**이 나오는지 확인한다. (id=42 "beep sound"는 실수집처이므로
검증에 쓰지 않는다.)
...
| 015·024·045·049 업로드 | 대기 | 각 파일의 라벨 조건(각도·거리·기둥·벽 등)을 사용자에게 받아야 함 |
...
| 45 | 탐지 시험 (본녹음 샘플) | height=3.0 / gap=8.0 | 034 → 10조각 (`A1_D0_L1_P0_W1_K1_001~010.wav`) |
