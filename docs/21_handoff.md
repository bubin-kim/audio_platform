...
### 3-12. 1일차 54개 전수 검증 → 주기 방식으로 전환 (2026-08-28)

**임계값 방식이 1개 파일에만 맞춰져 있었다.** 260818_001로 튜닝한
파라미터를 1일차 54개에 돌려보니 **정상 7개(13%)**, 0개 검출 12개,
1~5개 24개. 파일마다 신호 세기가 달랐던 것이 원인 — 260818_001은
배경 대비 강했지만(분리 여유 1.62배), 거리·각도가 다른 파일은 **진짜
비프음이 잡음 상위값보다 작았다**(0.25~0.84배).

**파라미터 100개 조합 전수 스윕으로 한계를 확인**: 9개 파일 GT
(각 파일의 실제 위상을 주기 접기로 측정해 구성) 기준 최선이
**F1 65.1%**(TP57/FP28/FN33). 임계값으로는 해결 불가능.

**해결: 주기 누적**(docs/16 §7 추가5). 임계값을 안 쓰고 "10초마다
반복되는 위치"를 찾는다. 접은 곡선 대비가 3.7~64배로 뛰어, 임계값
0개였던 파일들이 전부 10/10 정확해졌다.

**판정 지표도 고쳤다**(추가6): 위상이 0초 근처인 3개 파일이 오프셋
`%10`이 0.04/9.99로 갈려 표준편차 4.6초로 오판됐다 — 원형 표준편차로
바꿔 0.24~0.34초로 정상 판정.
...
**교훈(반복하지 말 것)**: ① 파일 1개로 파라미터를 정하지 말 것 —
조건(거리·각도)이 다양한 표본으로 확인해야 한다. ② "임계값을 못
넘는다"는 문제는 임계값 조정이 아니라 **다른 정보(주기·다채널 등)를
쓰는 것**으로 풀릴 수 있다. ③ 판정 지표 자체가 틀렸을 가능성을
의심할 것(원형 위상을 선형 통계로 재던 실수).

**배포 사고**: scp 실행 여부를 확인하지 않고 재검증을 돌려 **옛 코드로
측정한 무의미한 결과**를 30분간 수집했다. docs §5 항목24에 이미
기록해둔 함정("전송 완료라는 말만 믿지 말고 grep으로 확인")을 그대로
반복한 것 — 배포 후에는 **반드시 API 실동작으로 반영을 확인한 뒤**
검증을 시작할 것.

### 3-13. 검증 대격변 — "100%"의 정정, 앰비소닉 발견, 빔포밍 (2026-09-02~19)

> **이 절은 9월 세션 전체의 압축본이다.** 상세 근거·수치·기각 기록은
> docs/17 §2m~§2r, docs/18 §5~§6에 있다. 여기서는 "무엇이 확정됐고,
> 무엇이 미완이며, 월요일에 뭘 해야 하는지"를 잇는 데 필요한 것만 적는다.

#### (a) 확정된 사실 — 이후 모든 판단의 전제

| 사실 | 근거 |
|---|---|
| **"216/216 100%"는 틀린 보고였다** — 자동 기준(10개 검출+위상 sd≤0.5) 통과율일 뿐. 주기 방식은 신호가 없어도 구조적으로 통과한다 | 사용자가 260818_030을 귀로 듣고 "5초에 들린다" 지적(알고리즘은 1.83초). docs/17 §2m |
| **2,160개 전부에 비프음이 실재한다** — "검출 실패 677개"는 소리가 없는 게 아니라 소음에 묻혀 못 찾은 것. **음성(negative) 학습 샘플로 쓰면 안 된다** | 수집 설계(전 구간 키 누름). 보고서에서 "대조군 활용"이라 썼다가 사용자 지적으로 정정 |
| **마이크는 Zoom H3-VR 앰비소닉(AmbiX)** — 4채널은 별개 마이크가 아니라 W(전방향)/Y(좌우)/Z(상하)/X(앞뒤) 성분. **채널 순서가 W·Y·Z·X**로 일반 표기(W·X·Y·Z)와 다르다(헤더 실측 `zTRK1=W zTRK2=Y zTRK3=Z zTRK4=X`) | docs/18 §5. 이걸 모르고 "채널 정합성 깨짐"으로 오진했었다 |
| **채널 간 딜레이는 개념 자체가 성립 안 함** — 캡슐 간격 ~2cm(0.06ms) + 이미 합성된 신호 | docs/18 §5 |
| **게인 = 10/100 (공장 초기값)** — 파일에 기록 안 됨(기기에서만 확인 가능). 비프음 -63dBFS의 직접 원인이지만 **검출 성능의 원인은 아니다** | docs/18 §6 |
| **검출을 좌우하는 건 게인이 아니라 타겟 대역 SNR** — 3일차는 가장 작게 녹음(-76.9dB RMS)했는데 SNR 44.4dB로 검출 99.4%, 4일차는 SNR 12.7dB로 30.4%. 대역 배경이 3일차 대비 4일차 128배·1일차 943배 | docs/18. 게인은 신호·배경을 함께 키워 SNR 불변 |
| 비프음 지문 주파수 **2,003.9Hz ± 2.9Hz** (30표본 100%가 1950~2050Hz 안) — 진폭은 배경의 0.85배로 더 작지만 주파수는 완전히 갈린다(소음 피크 586Hz) | docs/17 §2n. 크기 기반 검출이 원리적으로 불가능한 이유 |

#### (b) 확정 산출물

| 산출물 | 위치 | 내용 |
|---|---|---|
| 라벨 v1 | `backend/labels/beep_labels_v1.json` (커밋됨) | 2,160 세그먼트: 검출시각·지문개수·오차·순음성·등급. **등급은 청취 미검증 초안**(메타에 명시) |
| 등급 분포 | 위 파일 | high 1,298(60.1%) / medium 114 / low 71 / none 677(31.3%). 일차별 high: 1일차 42% · 2일차 68% · **3일차 99.4%** · 4일차 30% |
| high 정확도 | docs/17 §2p | ±0.05초 **93.9%**, ±0.6초 96.2% — "알고리즘은 94% 정확, 데이터의 31%가 소음에 묻힘"으로 **정확도와 사용가능률을 분리**해 말할 것 |
| 청취 시트 | `~/Desktop/청취검증_시트.csv` (**저장소 밖!**) | 2,160행: 파일명·온셋·오프셋·(공란)청취한 초. 정렬: 260818→260819→260820→2_260820→270821→2_260821, 각 그룹 번호순. **연구원이 채우는 중** — 이게 모든 미결 판단의 선행 조건 |
| 보고서 2편 | `~/Desktop/미팅보고서.md`, `~/Desktop/알고리즘_논문서칭_보고서.md` (**저장소 밖!**) | 교수 보고용. 계층 번호(1. / 1.1) 형식 |

#### (c) 청취 검증 20개 (블라인드) — 첫 사람 정답

등급별 5개씩 60배 증폭, 번호 무작위. 결과: **지문 22회 이상 4개는 전부
들림(신뢰 가능)**, 그 아래는 들림/안들림이 뒤섞여 어떤 임계값도 최대 61%.
지문 9회인데 안 들리는 사례(2kHz 지속 기계음이 지문을 쌓음), 0회인데
들리는 사례(신호가 약해 PSD 피크 미달) 각각 존재. 위치 오류 2건도 귀로
확인됨("클릭 한참 후 비프음" 등). 상세: docs/17 §2m 이후.

#### (d) 빔포밍 — 검증 완료 + 코드 적용 완료, **NAS 배포만 남음**

- 실측(NAS 실데이터): "검출 실패" 40개 재검사 → **21개(52%) 검출 성공 전환**
  (677개 중 ~355개 복구 추정). 4일차 약한 신호 30개 SNR 19.2→21.5dB(26/30 개선).
- **기각: W 채널 단독** — 합성 실험에선 +10.6dB 최선이었으나 **실데이터에선
  평균보다 2.3dB 나쁨**. 비프음 순간엔 채널 상관이 +0.35~0.80으로 높아(한
  방향 음원) 평균이 신호를 보강한다. **합성 신호 결론을 실데이터 검증 없이
  채택하지 말 것** — 이번 세션에서 합성 실험이 두 번 배신했다(소음을
  점음원으로 두면 평균이 최선, 확산음으로 두면 W가 최선 — 둘 다 실제와 다름).
- 구현: `audio/ambisonic_beam.py` + 검출기 `_to_analysis_mono()`.
  파라미터 `beam`(기본 **None=기존 4채널 평균, 회귀 테스트로 100% 동일 보장**)
  / `"auto"`=대역 에너지 최대 방향 자동 탐색 / 숫자=방위각.
  API: `GET /api/source-files/{id}/beep-onsets?beam=auto`.
  라벨의 각도 코드는 **0° 물리 기준이 미기록**이라 쓰지 않고 auto 탐색을 쓴다.
- 커밋 `36055fd`. 신규 테스트 13개 포함 전체 306개 통과, 로컬 스모크 통과.

#### (e) 다중 특징 판정 — 구현했으나 **미채택** (커밋 `a54ee0a`)

`audio/beep_grading.py`(지문+순음성+주파수정확도+지속시간). 청취 정답 18개
대비 55.6%→61.1%로 1개 차이 — 무의미. 원인은 특징 분포 자체가 겹침
(들림/안들림의 순음성 중앙 14.8 vs 13.8dB). **기존 판정 유지.**
결론엔 알고리즘이 아니라 **애매 구간(지문 0~9) 청취 정답 30~50개**가 필요
— 청취 시트가 채워지면 `evaluate_against_listening()`으로 즉시 재평가.

#### (f) 월요일 출근 시 할 일 (순서대로)

1. **빔포밍 배포** — 맥에서:
   ```bash
   cd /Users/bbin/Documents/audio_platform
   tar czf /tmp/beam.tar.gz backend/app/audio/ambisonic_beam.py \
     backend/app/audio/band_beep_detector.py backend/app/audio/beep_grading.py \
     backend/app/services/upload_service.py backend/app/api/routes/uploads.py \
     backend/tests/test_ambisonic_beam.py backend/tests/test_band_beep_detector.py \
     frontend/components/datasets/SourceList.tsx
   scp -O /tmp/beam.tar.gz aura_admin@203.247.33.93:/volume1/AURA/01_Projects/01_BeepSound/03_DashBoard/
   ```
   NAS에서:
   ```bash
   cd /volume1/AURA/01_Projects/01_BeepSound/03_DashBoard
   sudo tar xzf beam.tar.gz
   sudo docker compose up -d --build --force-recreate backend frontend
   ```
   ⚠️ `--force-recreate` 없으면 캐시로 구버전이 계속 돈다(§5 항목31).
   tar의 `LIBARCHIVE.xattr` 경고는 무해. **배포 판정은 로그가 아니라 API로**:
   `curl -s -H "Authorization: Bearer aura26" "http://203.247.33.93:8100/openapi.json" | grep -o beam`
2. **빔포밍 실서버 확인** — `?beam=auto`로 030·050 등 실패 파일 몇 개 재검출,
   지문 개수가 늘어나는지 확인.
3. 청취 시트 진행 상황 확인 → 채워진 만큼 `beep_grading.evaluate_against_listening()` 재평가.
4. (여유 시) "검출 실패" 677개 전수에 beam=auto 돌려 실제 복구 수 확정
   → 355개 추정치를 실측으로 교체.

#### (g) 이 구간에서 추가로 데인 것 (→§5에도 반영)

- **분모를 잘못 잡으면 결론이 뒤집힌다**: 4일차 "성공 33%"는 비프음 없는
  세그먼트까지 분모에 넣은 수치였다. 지문 있는 82개만 보면 84%.
- **`abs()` 값을 부호 있는 값으로 착각**해 "오차 13개 전부 + 방향 편향"이라
  보고했다가 정정(실제 양수 7·음수 6).
- **지문 1개와 6개를 같은 '존재'로 취급**하지 말 것 — 진짜 비프음(~180ms)은
  50ms 스캔에서 연속 4~6개가 잡힌다. 1개는 우연한 튐일 가능성.
- 순음성 14dB 기준은 미검증 파일 1개에서 나온 값 → **근거 순환**. 지문
  주파수(30표본 독립 확인)처럼 데이터가 스스로 가리키는 값을 쓸 것.
...
32. **`docker compose up -d --build`만으로는 새 코드가 반영 안 될 수 있다**
    (2026-09-02 실제 발생): scp로 파일을 보내고 재빌드해도 이미지 레이어
    캐시가 유효하면 재빌드를 건너뛴다. 컨테이너는 `Started`로 뜨고 로그도
    정상이라 배포된 것처럼 보인다. **`--force-recreate`를 붙일 것.**
    배포 판정은 로그가 아니라 **API로**(openapi.json에 신규 경로가 있는지
    grep). 로그 타임스탬프는 재시작해도 과거 것이 보여 신뢰 불가.
    macOS tar를 NAS에서 풀 때 나오는 `LIBARCHIVE.xattr` 경고는 무해하다.

33. **합성 신호 실험 결과를 실데이터 검증 없이 채택하지 말 것**
    (2026-09-14 두 번 배신당함): 소음을 점음원으로 두면 "4채널 평균이
    최선", 확산음으로 두면 "W 단독이 +10.6dB 최선"이라는 결론이 나왔는데
    **둘 다 실데이터와 달랐다**(실데이터: 빔포밍 > 평균 > W). 비프음
    순간엔 채널 상관이 +0.35~0.80으로 높다는 실측이 결정타였다.
    소음 모델 하나로 결론이 뒤집히는 실험은 근거가 못 된다.

34. **자동 판정 기준의 통과율을 정확도로 보고하지 말 것** (2026-09-02,
    이 프로젝트 최대 사고): "216/216 100%"는 주기 방식이 구조적으로 항상
    통과하는 기준의 통과율이었고, 실제로는 55~60%였다. §2g에 그 함정을
    직접 적어놓고도 판정에 그 기준을 썼다. **내가 만든 기준으로 내 결과를
    검증하는 순환**(순음성 14dB도 미검증 파일 1개 유래)을 늘 의심할 것.
    발각 경로는 사용자의 귀였다 — 청취 정답 없이는 무엇도 확정이 아니다.
