...
24. **NAS `data/uploads/{id}`·`data/segments/{id}` 폴더명을 File Station에서
    ...
    다음에 이 증상을 보면 먼저 `data/uploads/`·`data/segments/` 아래가
    전부 숫자 폴더인지 확인할 것. 폴더명에 사람이 읽는 이름을 넣고
    싶으면 디스크에서 임의로 바꾸지 말고 코드(`storage_path` 생성 규칙)를
    먼저 고칠 것 — docs/17 §2l에 상세 기록.
