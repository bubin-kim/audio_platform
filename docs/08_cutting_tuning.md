...
### 실전 사례 3: 차량 경적 19회 100초 (2026-07-17, 이벤트 수 정합 검증)

- 파일: horn_19events_100s.wav (44.1kHz, 경적 ~2.3초 + 간격 ~5.2초 × 19회)
- 측정: 바닥 p10 **-34.2 dBFS** / 신호 p90 **-11.9 dBFS** (차이 22dB — silence_based 적합).
- 추천 **-23dBFS** (중간값) 적용 → 시뮬레이션·실커팅 모두 **정확히 19조각**,
  세그먼트 시작시각이 시뮬레이션과 ±0.000s 일치, 이벤트 간격 4.91~5.63s로 규칙적
  (이중 검출·미탐 없음). 파일럿(-25) 값과 -23 모두 동일 결과 — 바닥·신호 사이
  마진이 넓어 ±2dB는 둔감.
- 품질 검사(expected_segments_per_source=19)도 ok — 기대 수 자동 대조에 활용 가능.
