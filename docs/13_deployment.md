...
| 결정할 것 | 측정 방법 | 반영처 | **실측 결과 (2026-07-15, DP-M4)** |
|---|---|---|---|
| Railway 프록시 업로드 한도 | 배포 직후 50/100/200MB 더미 업로드 실측 | `MAX_UPLOAD_MB` 기본값 | 50MB → 201, 14초. 프록시 차단 없음 확인 — `MAX_UPLOAD_MB=200` 유지 (100/200MB는 필요 시 추가 실측) |
| 재생 첫 로드 지연(Drive 미스) | 배포 환경에서 세그먼트 재생 실측 | 캐시 크기·프리페치 필요성 판단 | waveform 미스 1.07초 → 캐시 히트 0.58초. audio 첫 로드 3.7초(529KB) — 반복 청취는 캐시 흡수, 프리페치 불필요 판단 |
| 커팅 Job 총 시간(Drive 저장 포함) | 3분 파일 1개 실측 (조각 ~30개) | UI 안내 문구·타임아웃 값 | **83초** (조각당 ~2.7초, Drive 동기 저장 포함). 로컬 대비 ~40배지만 백그라운드 Job이라 UX 무영향 |
| Railway 임시 디스크 가용량 | 배포 후 `df` 실측 | `CACHE_MAX_MB` 기본값 조정 | **미실측** (CLI ssh 미지원 플랜) — 캐시 LRU 상한 2048MB로 시작, 디스크 경고 발생 시 조정 |
