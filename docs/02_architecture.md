...
(6) CSV 생성
    Dataset 단위로 Segment들을 Metadata.csv 로 내보내기
        │
(7) 이벤트 훅 발화  ※ 지금은 아무도 안 듣는다 (V2에서 Notion/Drive가 구독)
        │
(8) 대시보드
...
MVP엔 비어 있지만, V2 기능이 **정확히 어디에 꽂히는지**를 지금 못박아 둔다.

```
업로드 완료 ──▶ [HOOK: on_upload_complete]  ──▶ (V2) Notion "데이터 추가됨" 기록
커팅 완료  ──▶ [HOOK: on_processing_done]  ──▶ (V2) Drive 업로드 + Notion 연구일지
CSV 생성  ──▶ [HOOK: on_dataset_exported]  ──▶ (V2) GitHub 커밋(Dataset 버전)
...
MVP에서는 우체통만 설치하고 아무도 편지를 안 읽는다.
V2에서 Notion 구독자가 생기면, 그 우체통에서 편지를 꺼내 Notion에 기록한다.
→ **핵심 로직(커팅·저장)은 Notion/Drive의 존재를 전혀 모른다.** 그래서 MCP 없이도 완벽히 동작한다.
