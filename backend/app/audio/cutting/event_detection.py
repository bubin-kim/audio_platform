...
**알고리즘** (사용자 청취 GT로 검증, 2026-08-26 갱신):
```
1. 채널 평균 (mean)                      ← GT 검증: ch3 단독 35.5% < mean 58.1%
2. 리샘플 48k → 44.1k (scipy.signal.resample)
3. STFT (nperseg=4096, noverlap=2048)    ← 2048에서 확대 (§2j)
...
4번을 평균→최대로 바꾼 것과 3번의 nperseg 확대가 성능을 갈랐다. 찾는
이벤트가 좁은 대역의 순음이라 대역 평균은 신호를 희석하고, 주파수
해상도가 낮으면(빈 폭이 넓으면) 순음이 주변 잡음과 덜 분리된다.

**검증 결과** (004 GT 31개, tolerance 1.5초, 파라미터 동일):

| 설정 | TP | FP | FN | F1 |
|---|---|---|---|---|
| 평균 집계 + nperseg 2048(구) | 24 | 7 | 7 | 77.4% |
| 최대 집계 + nperseg 2048 | 29 | 2 | 2 | 93.5% |
| **최대 집계 + nperseg 4096(현)** | **30** | **0** | **1** | **98.4%** |

본녹음 5개 A/B 청취 판정에서도 최대값이 이겼다 — 평균이 단독 검출한
지점(045 9.1초·20.9초, 024 1.2초)은 전부 경보음이 없었다(§2i).
nperseg 4096은 본수집 3일차 실제 사례(018번 55초 근방 인접 이벤트 병합,
029번 84·100초 근방 약한 오탐)를 직접 해결하는 것을 확인 후 채택했다(§2j).
...
params (전부 Project 설정 — 도메인 값은 코드에 없다, P1):
  ...
  - nperseg (4096) / noverlap (2048): STFT 주파수/시간 해상도. 크게 하면
    주파수 해상도↑(순음 분리 잘됨) 시간 해상도↓(위치 정밀도 하락) — 6초+
    지속되는 경보음에는 이 트레이드오프가 유리했다(§2j)
...
DEFAULTS: dict[str, Any] = {
    ...
    "analysis_sr": 44100,
    "channel": "mean",
    "nperseg": 4096,  # 2026-08-26: 2048→4096, GT 검증(§2j)으로 채택
    "noverlap": 2048,
}
